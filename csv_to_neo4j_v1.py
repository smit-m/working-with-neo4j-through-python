        ...
        try:
            ...
            for row in reader:    
                if row:
                    userid = strip(row[0])
                    brand = strip(row[1])
                    category = strip(row[2])
                    url = strip(row[3])
                    site = strip(row[4])
                    price = strip(row[5])
                    productName = strip(row[6])
                    productid = strip(row[7])
                    eventtype = strip(row[8])
                    
                    query = """
                        merge (userid:User {User: {u}})
                        merge (brand:Brand{Brand:{b}})
                        merge (productName:productName {productName: {pn}})
                        
                        merge (u)-[:Clicked_ON]->(pn)-[:Is_Made_By]->(b)
                    """
